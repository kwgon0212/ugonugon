    ...
    <>
      ...
      <Main hasBottomNav={true}>
        <form className="w-full p-layout flex flex-col gap-layout divide-[#0b798b]">
          ...
          <ul className="flex w-full h-10 gap-x-[5px] mb-[10px] list-none relative">
            {Object.keys(hireType).map((value, index) => (
              <li
                key={index}
                className="btn w-1/3 text-sm border-main-gray bg-white rounded-[10px] text-main-darkGray"
                onClick={handleClick}
              >
                {value}
              </li>
