...
router.get("/userInfo/:userId", async (req, res) => {
  try {
    console.log("api 들어옴");
    const data = await Users.findById(req.params.userId).select("noticeIds");
    if (!data) {
      return res.status(404).json({ error: "User 를 찾을 수 없음" });
    }
    const noticeIds = Array.isArray(data.noticeIds)
      ? data.noticeIds
      : [data.noticeIds];
    res.status(200).json({ noticeIds });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
});
